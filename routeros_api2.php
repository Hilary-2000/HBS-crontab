...
class routeros_api2
{
     ...
     */function read($parse = true)
    {
        $RESPONSE = array();
        $start_time = time();
        $timeout = isset($this->timeout) ? (int)$this->timeout : 3; // use class timeout if defined

        while (true) {
            // ⏱️ Check total elapsed time
            if ((time() - $start_time) > $timeout) {
                $this->debug("⚠️ Read timeout reached after {$timeout}s — exiting read loop.");
                break;
            }
            ...
            if ($BYTE === false || $BYTE === '') {
                ...
                $this->debug("⚠️ No data from socket. Stream info: " . json_encode($info));
                break;
            }
            ...
            if ($LENGTH > 0) {
                ...
                // Apply per-read timeout to stream
                stream_set_timeout($this->socket, $timeout);

                while ($retlen < $LENGTH) {
                    if ((time() - $start_time) > $timeout) {
                        $this->debug("⚠️ Timeout while reading chunk after {$timeout}s");
                    ...
                    if ($chunk === false || $chunk === '') {
                        $info = stream_get_meta_data($this->socket);
                        $this->debug("⚠️ fread() failed or empty. Stream info: " . json_encode($info));
                        break;
                    }
            ...
            // Safety break to prevent endless reads
            usleep(50000); // 50ms sleep to prevent CPU spikes
        }
